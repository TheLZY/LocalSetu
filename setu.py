...
"""
# @sv.on_fullmatch(('setuhelp','色图帮助','setu帮助','LocalSetu','涩图帮助'))
...
@sv.on_fullmatch(('kkqyxp','看看群友xp','看看群友性癖','kkntxp','看看男同xp','看看男同性癖','xp','kkxp'))
async def choose_setu(bot, ev):
    ...
    try:
        ...
        # Test 输出名字
        # 方式：bot.get_group_list() get_group_member_info() 
        # 那就得先try一下了
        ...
        gid = ev['group_id']
        try:
            upinfo = await bot.get_group_member_info(group_id=gid, user_id=str(user))
            up = upinfo['card']
        except:
            upinfo = await bot.get_stranger_info(user_id=str(user))
            up = upinfo['nickname']
        ...
        for seg in ev.message:
            if seg.type == 'text':
                ...
                if not result:
                    ...
                    # 直接过审
                    sql="INSERT OR IGNORE INTO LocalSetu (id,url,user,date,tag,man,verify) VALUES (NULL,?,?,datetime('now'),?,?,0)"
                    cursor.execute(sql,(setu_name,user,tag,is_man))
...
async def del_setu(bot, ev: CQEvent):
    ...
    if int(user) in verifies or priv.check_priv(ev, priv.ADMIN):
        try:
            test_conn()
...
async def verify_complete(bot, ev: CQEvent):
    ...
    for i,raw in enumerate(results):
        user = raw[0]
        number = raw[1]
        text = text +f"\n第{str(i+1)}名："+ f'[by {str(user)}] '+f'上传{str(number)}张'
